...
def test_root_serves_self_contained_zero_install_ui() -> None:
    ...
    assert 'id="draft-fidelity-status"' in response.text
    assert 'id="draft-fidelity-detail"' in response.text
    assert 'id="draft-fidelity-findings"' in response.text
...
def test_web_ui_exposes_ai_status_contradictions_and_reviewable_draft() -> None:
    ...
    assert "aiReview.draft_fidelity" in WEB_UI_HTML
    assert 'payload.draft_fidelity.status !== "PASS"' in WEB_UI_HTML
    assert "Unsafe AI draft cannot be loaded as input" in WEB_UI_HTML
